    ...
    return (
      <div>
        <div className="flex flex-col gap-2">
          <div>
            {/* Main File Input */}
            {value?.some(file => file.progress !== 'COMPLETE') ? null : (
              <div
                {...getRootProps({
          ...
          {value?.length !== 0 && (
            <ScrollArea 
              className={cn(
                value?.some(file => file.progress !== 'COMPLETE') ? 'h-24 py-3 px-1' : 'h-0',
            ...
            >
              {value?.map(({ file, progress }, i) => (
                progress === 'COMPLETE' ? null : (
                  <div
                    key={i}
                    className="flex h-16 w-96 max-w-[100vw] flex-col justify-center rounded border px-4 py-2"
                  >
